...
const BussinessEventHistoryScreen = ({ navigation }) => {
  ...
  const getMyallEvents = async() => {
    setIsloading(true)
    const result =  await dispatch(eventAction.getBusinessHistory(user_data))
    ...
    <>
      <View style={styles.wrapper1}>
        ...
        <ScrollView style={{ marginTop: windowHeight / 9.5 }}>
          <View style={{
            minHeight:!bookingHistory?.length>0?windowHeight/2.8:10
          }}>
          <Text
            style={{
              color: "white",
          ...
          {bookingHistory?.length>0?<Text></Text>:<Text style={{
            color:"white",
            marginLeft: windowWidth / 30,
            fontSize: normalizeFont(14),
            marginTop:20
        }} >No Event History!</Text>}
          </View>
         ...
         {console.log("bookingHistory",bookingHistory)}
          ...
          bookingHistory?.map((item) => { 
            ...
            return (
             ...
             screenName={"BussinessEventHistoryDetails"} 
          />
            )
...
export default BussinessEventHistoryScreen;
