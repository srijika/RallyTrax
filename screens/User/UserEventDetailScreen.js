...
const UserEventDetailScreen = ({ route }) => {
    ...
    <>
      ...
      <View style={styles.wrapper1}>
        <View style={styles.wrapper2}>
          <Image
            style={{ marginTop: -w / 55.15, }}
            source={require('../../assets/images/f1.png')} />

        </View>
        <TouchableOpacity style={{ zIndex: 1, position: 'absolute', top: w / 17 }} onPress={() => navigation.goBack()}>
          <ArrowBack />
        </TouchableOpacity>
        ...
        <ScrollView style={{ marginTop: w / 5.5 }}>
          ...
          <ScrollView nestedScrollEnabled={true} style={{ padding: 20, maxHeight: w / 4, minHeight: w / 4 }} >
            <View >
          ...
          {!isLoading ? (
            <TouchableOpacity
            style={[styles.loginButton, { marginBottom: w / 25 }]}
            onPress={() => {
          ...
          >
            ...
            </TouchableOpacity>) : (<ActivityIndicator style={[styles.loginButton, { marginBottom: w / 25 }]} size="small" color="white" />)}
        </View>
...
const styles = StyleSheet.create({
  ...
  loginButton: {
    width: "95%", alignSelf: "center", borderRadius: 30, backgroundColor: "#5b0b0b", padding: windowWidth / 18, marginTop: w / 60, fontWeight: '700',
  },
  buttonText: {
