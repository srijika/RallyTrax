...
const UserSettingScreen = ({ navigation }) => {
  ...
  const termsAndConditions = pages?.filter(obj => {
    return obj.slug === 'terms-and-conditions'
  });
  const PrivacyPolicy = pages?.filter(obj => {
    return obj.slug === 'privacy-policy'
  });
  const aboutUs = pages?.filter(obj => {
    return obj.slug === 'about-us'
  });

  const handleRedirect = (ScreenName, page, data) => {
  ...
  return (
    <>
      <View style={styles.wrapper1}>
        ...
        onPress={() => { handleRedirect("StaticPageScreen", "term_condtions",termsAndConditions) }}
        >
        ...
        <TouchableOpacity style={styles.buttonStyle}
          // onPress={() => { navigation.navigate('UserPrivacyPolicyScreen') }}
          onPress={() => { handleRedirect("StaticPageScreen", "privacy_policy", PrivacyPolicy) }}
        >
        ...
        <TouchableOpacity style={styles.buttonStyle}
          onPress={() => { handleRedirect("StaticPageScreen", "about_us",aboutUs) }}
        >
...
const styles = StyleSheet.create({
  ...
  buttonStyle: { width: '90%', alignSelf: 'center', borderBottomWidth: 1, borderColor: '#7a7c7c' }
})